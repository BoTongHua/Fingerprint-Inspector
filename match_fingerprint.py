...
def match(path):
    """
    A method takes path to the to be inspected fingerprint file.

    example path: C:\\program files\\data\\fingerprint.BMP
    """
    sample = cv2.imread(str(path))
    ...
    cv2.imshow(f"Result: {filename} | Best score: {best_score}", result)
    cv2.waitKey(0)
